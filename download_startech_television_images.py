#!/usr/bin/env python3
"""
Download ALL Star Tech Television Shop primary product images into hierarchy folders.
...
Examples:
    ...
    # Mapping only
    python download_startech_television_images.py --project-root . --dry-run

    # Test first 10
    python download_startech_television_images.py --project-root . --limit 10

    # Download all current television-shop products
    python download_startech_television_images.py --project-root .

    # Download one branch
    python download_startech_television_images.py --project-root . --category smart-tv
    python download_startech_television_images.py --project-root . --category 4k-tv
    python download_startech_television_images.py --project-root . --category tv-stand-wall-mount

    # Re-download existing images
    python download_startech_television_images.py --project-root . --overwrite

Notes:
- Product count is NOT hardcoded.
- Pagination is read live from Star Tech.
- Existing images are skipped unless --overwrite is used.
- Products are classified by matching Star Tech Television category pages.
- A manifest is continuously written to:
      startech_television_image_manifest.json
"""
...
def find_category_url(client: Client, label: str) -> str | None:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "television" in path or "tv" in path:
            score += 30
...
def main() -> int:
    ...
    ap.add_argument("--manifest", default="startech_television_image_manifest.json")
    ...
    print(f"Reading Television master catalog: {TV_URL}")
    master, reported_total = crawl(client, TV_URL, verbose=True)
    ...
    print("\nDiscovering Television category URLs...")
    nodes = []
    ...
    for order, (slug, label) in enumerate(CATEGORY_LABELS.items(), start=1):
        ...
        print(f"  Television > {label}: {url or 'NOT FOUND'} ({len(matched)} matched)")
        nodes.append({
            "slug": slug,
            "name": label,
            "url": url,
