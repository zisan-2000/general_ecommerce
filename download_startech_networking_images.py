...
Current direct categories include:
    Starlink
    Router
    Pocket Router
    WiFi Range Extender
...
Examples:
    ...
    # Mapping only
    python download_startech_networking_images.py --project-root . --dry-run

    # Test first 10
    python download_startech_networking_images.py --project-root . --limit 10

    # Download all current /networking products
    python download_startech_networking_images.py --project-root .

    # Download one branch
    python download_startech_networking_images.py --project-root . --category router
    python download_startech_networking_images.py --project-root . --category network-switch
    python download_startech_networking_images.py --project-root . --category networking-cable

    # Re-download existing images
    python download_startech_networking_images.py --project-root . --overwrite
"""
...
CATEGORY_LABELS = OrderedDict([
    ...
    ("network-transceivers", "Network Transceivers"),
    ...
    ("splicer-machine", "Splicer Machine"),
...
def find_category_url(client: Client, label: str) -> str | None:
    soup = client.soup(NETWORKING_URL)
...
def main() -> int:
    ...
    ap.add_argument("--manifest", default="startech_networking_image_manifest.json")
    ...
    print(f"Reading Networking master catalog: {NETWORKING_URL}")
    master, reported_total = crawl(client, NETWORKING_URL, verbose=True)
    ...
    print("\nDiscovering Networking category URLs...")
    nodes = []
    for order,(slug,label) in enumerate(CATEGORY_LABELS.items(), start=1):
        ...
        print(f"  Networking > {label}: {url or 'NOT FOUND'} ({len(matched)} matched)")
