...
CATEGORY_URL = f"{BASE}/ups-battery"
...
def crawl(client: Client):
    ...
    while True:
        ...
        print(f"page {page}: {len(batch)} cards, {len(products)-before} new, total {len(products)}" + (f"/{total}" if total else ""))
...
def main():
    ...
    ap.add_argument("--output-root", default="public/images/products/power/ups-battery")
    ap.add_argument("--manifest", default="startech_ups_battery_image_manifest.json")
    ...
    manifest = {
        ...
        "categoryPath": ["power", "ups-battery"],
    ...
    for idx, (product_url, name) in enumerate(selected, start=1):
        ...
        if args.dry_run:
            rec["status"] = "dry-run"
            print(f"{progress} MAP power/ups-battery <- {name}")
