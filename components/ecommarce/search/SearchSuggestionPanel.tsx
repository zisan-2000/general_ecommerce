...
const formatPrice = (value: number, currency: string, locale: string) =>
  `${currency === "BDT" ? "৳" : `${currency} `}${Math.round(value).toLocaleString(locale)}`;
...
export default function SearchSuggestionPanel({
  ...
  const t = useTranslations("StorefrontShell.search");
  ...
  return (
    ...
    >
          ...
          {relatedLinks ? (
            <section className="grid gap-3 border-t border-border bg-muted/35 p-3 sm:grid-cols-2">
              {data?.categories.length ? (
                <div>
                  <p className="mb-1.5 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-muted-foreground">
                    <FolderSearch2 className="h-3.5 w-3.5" /> {t("categories")}
                  </p>
              ...
              {data?.brands.length ? (
                <div>
                  <p className="mb-1.5 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-muted-foreground">
                    <Tag className="h-3.5 w-3.5" /> {t("brands")}
                  </p>
                  <div className="flex flex-wrap gap-1.5">
              ...
              {data?.suggestedQueries.length ? (
                <div className="sm:col-span-2">
                  <p className="mb-1.5 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-muted-foreground">
                    <Search className="h-3.5 w-3.5" /> {t("popularSearches")}
                  </p>
                  <div className="flex flex-wrap gap-1.5">
      ...
      {data?.query && !loading ? (
        ...
        >
          <Search className="h-4 w-4" /> {t("viewAllResults", { query: data.query })}
        </button>
      ) : null}
    </div>
  );
