...
Examples:
    ...
    # Mapping only
    python download_startech_software_images.py --project-root . --dry-run

    # Test first 10
    python download_startech_software_images.py --project-root . --limit 10

    # Download all current /software products
    python download_startech_software_images.py --project-root .

    # Download one branch
    python download_startech_software_images.py --project-root . --category antivirus
    python download_startech_software_images.py --project-root . --category adobe
    python download_startech_software_images.py --project-root . --category operating-system

    # Re-download existing files
    python download_startech_software_images.py --project-root . --overwrite
"""
...
CATEGORY_LABELS = OrderedDict([
    ...
    ("mail-server-solution", "Mail Server Solution"),
    ("cloud-solutions", "Cloud Solutions"),
    ("antivirus", "Antivirus"),
    ("bangla-typing-software", "Bangla Typing Software"),
    ("adobe", "Adobe"),
    ("vmware", "VMware"),
    ("autodesk", "AutoDesk"),
    ("anydesk", "AnyDesk"),
...
def main() -> int:
    ...
    ap.add_argument("--manifest", default="startech_software_image_manifest.json")
    ...
    print(f"Reading Software master catalog: {SOFTWARE_URL}")
    master, reported_total = crawl(client, SOFTWARE_URL, verbose=True)
    ...
    print("\nDiscovering Software category URLs...")
    nodes = []
    ...
    for order,(slug,label) in enumerate(CATEGORY_LABELS.items(), start=1):
        ...
        print(f"  Software > {label}: {url or 'NOT FOUND'} ({len(matched)} matched)")
    ...
    manifest = {
        "sourceCategoryUrl": SOFTWARE_URL,
