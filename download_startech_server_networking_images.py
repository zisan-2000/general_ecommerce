...
Examples:
    ...
    # Mapping only
    python download_startech_server_networking_images.py --project-root . --dry-run

    # Test first 10
    python download_startech_server_networking_images.py --project-root . --limit 10

    # Download all current /server-networking products
    python download_startech_server_networking_images.py --project-root .

    # Download one branch
    python download_startech_server_networking_images.py --project-root . --category server
    python download_startech_server_networking_images.py --project-root . --category server-rack
    python download_startech_server_networking_images.py --project-root . --category nas-storage

    # Re-download existing files
    python download_startech_server_networking_images.py --project-root . --overwrite
"""
...
CATEGORY_LABELS = OrderedDict([
    ...
    ("server-hdd-bay", "Server HDD Bay"),
    ("server-ram", "Server RAM"),
    ("server-ssd", "Server SSD"),
    ("server-power-supply", "Server Power Supply"),
])
...
def find_category_url(client: Client, label: str) -> str | None:
    ...
    for a in soup.find_all("a", href=True):
        ...
        score = 0
        if path.startswith("/server-networking/"):
            score += 50
...
def main() -> int:
    ...
    ap.add_argument("--manifest", default="startech_server_networking_image_manifest.json")
    ...
    print(f"Reading Server & Storage master catalog: {SERVER_URL}")
    master, reported_total = crawl(client, SERVER_URL, verbose=True)
    ...
    print("\nDiscovering Server & Storage category URLs...")
    nodes = []
    for order,(slug,label) in enumerate(CATEGORY_LABELS.items(), start=1):
        ...
        print(f"  Server & Storage > {label}: {url or 'NOT FOUND'} ({len(matched)} matched)")
