...
function upsertDocumentDraft(
  ...
  const index = nextDocuments.findIndex((document) => document.type === nextDocument.type);
...
function DocumentUploadCard({
  ...
  const t = useTranslations("AdminSuppliers.documents");
  ...
  if (hasPendingFile) {
    statusLabel = hasSavedFile ? t("replaceOnSave") : t("readyToUpload");
    statusClasses = "border-warning/20 bg-warning/10 text-warning";
  }
  if (isMarkedForRemoval) {
    statusLabel = t("removed");
      ...
      <div className="mt-3 sm:mt-4 space-y-3 text-sm">
        {hasSavedFile && (
          <div className="rounded-lg border border-border bg-muted/30 p-3">
            <p className="font-medium text-foreground text-sm">
              {draft?.fileName || t("currentDocument")}
            </p>
            <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
              {draft?.mimeType && <span>{draft.mimeType}</span>}
              {formatBytes(draft?.fileSize) && <span>{formatBytes(draft?.fileSize)}</span>}
            </div>
            <div className="mt-2">
        ...
        {hasPendingFile && (
          <div className="rounded-lg border border-warning/20 bg-warning/10 p-3">
            ...
            <p className="mt-1 text-xs text-warning/80">{t("willBeUploaded")}</p>
          </div>
        )}

        {isMarkedForRemoval && (
          <div className="rounded-lg border border-destructive/20 bg-destructive/10 p-3">
            <p className="font-medium text-destructive text-sm">{t("removedFromDraft")}</p>
            <p className="mt-1 text-xs text-destructive/80">{t("uploadReplacement")}</p>
          </div>
        )}
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <Input
              ...
              onChange={(event) => onFileChange(type, event.target.files?.[0] ?? null)}
            />
...
export default function SuppliersPage() {
  ...
  const loadSuppliers = async () => {
    try {
      ...
      const [data, categories] = await Promise.all([
        getJson<Supplier[]>(`/api/scm/suppliers${params.size ? `?${params.toString()}` : ""}`),
  ...
  const documentSummary = useMemo(() => {
    const currentDocuments = requiredDocumentTypes.map((type) => {
      ...
      return {
        type,
        fileUrl:
          draft && !draft.removed ? (draft.file ? "__pending__" : draft.fileUrl) : "",
      };
    });

    const missing = getMissingSupplierDocumentTypes(form.companyType, currentDocuments);
    ...
    return { missing, uploaded, progress, complete: missing.length === 0 };
  ...
  const handleDocumentFileChange = (type: SupplierDocumentType, file: File | null) => {
    if (!file) return;
  ...
  const createSupplierCategory = async (input?: {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload?.error || t("errors.createCategory"));
      }
  ...
  const saveSupplier = async (
    ...
    if (missingDocumentTypes.length > 0) {
      toast.error(
        `${t("documents.required")}: ${missingDocumentTypes
    ...
    <div className="min-h-screen space-y-4 sm:space-y-6 p-3 sm:p-4 md:p-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          <Button onClick={openCreateModal}>{t("actions.addSupplier")}</Button>
      ...
      <Card className="shadow-sm">
        <CardHeader className="p-4 sm:p-6">
          <CardTitle className="text-base sm:text-lg">{t("header.title")}</CardTitle>
        </CardHeader>
                  ...
                  {paginatedSuppliers.map((supplier) => (
                    <TableRow key={supplier.id} className="border-border hover:bg-muted/40">
                      ...
                      <TableCell className="py-3">
                        ...
                        >
                          {supplier.isActive ? t("filters.active") : t("filters.inactive")}
                        </Badge>
                      </TableCell>
                    ...
                    <div className="pt-2">
                      ...
                      >
                        {supplier.documentsComplete
                          ...
                          : `${supplier.uploadedRequiredDocumentCount}/${supplier.requiredDocumentCount} ${t("table.documents")}`}
                      </Badge>
                    </div>
