...
def main() -> int:
    ...
    ap.add_argument("--manifest", default="startech_gaming_image_manifest.json")
    ...
    print(f"Reading Gaming master catalog: {GAMING_URL}")
    master, reported_total = crawl(client, GAMING_URL, verbose=True)
    ...
    print("\nDiscovering Gaming category URLs...")
    nodes = []
    for order, (slug, label) in enumerate(CATEGORY_LABELS.items(), start=1):
        ...
        print(f"  Gaming > {label}: {url or 'NOT FOUND'} ({len(matched)} matched)")
        nodes.append({
            "slug": slug,
            "name": label,
            "url": url,
