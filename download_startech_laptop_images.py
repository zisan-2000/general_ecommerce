#!/usr/bin/env python3
"""
Download ALL Star Tech Laptop & Notebook product primary images into hierarchy folders.
...
Examples:
    ...
    python download_startech_laptop_images.py --project-root . --dry-run

    # Test first 10 products
    python download_startech_laptop_images.py --project-root . --limit 10

    # Download all Laptop product images
    python download_startech_laptop_images.py --project-root .

    # Download only one top-level component group
    python download_startech_laptop_images.py --project-root . --category gaming-laptop
    python download_startech_laptop_images.py --project-root . --category all-laptop

    # Replace existing images
    python download_startech_laptop_images.py --project-root . --overwrite
...
# Star Tech Laptop hierarchy supplied by the user and matching the current
# Laptop menu. URLs are discovered live rather than hardcoded.
CATEGORY_TREE = OrderedDict(
    [
        (
            "All Laptop",
            [
                "Lenovo", "MSI", "HP", "Asus", "Tecno", "Chuwi",
                "MacBook", "Gigabyte", "Acer", "Dell", "Microsoft",
                "Smart", "Thunderobot", "Walton",
            ],
        ),
        ("OLED Laptop", []),
        ...
        (
            "Laptop Bag",
            [
                "Asus", "Dell", "Fantech", "Lenovo", "MaxGreen", "BWOO",
                "Targus", "Tucano", "UGREEN", "WiWU", "Xiaomi",
                "Arctic Hunter", "Honor",
            ],
        ),
        (
            "Laptop Accessories",
            [
                "Laptop Cooler", "Laptop Desk", "Laptop RAM", "Laptop Stand",
                "Laptop Battery", "Laptop Charger / Adapter", "Display",
                "Laptop Keyboard", "Caddy",
            ],
        ),
        ("Pentium and Celeron Laptop", []),
...
def discover_nodes(client: Client) -> list[dict]:
    ...
    for top_order, (top_name, child_names) in enumerate(CATEGORY_TREE.items(), start=1):
        ...
        top_url = find_exact_link(client, LAPTOP_URL, top_name)

        if top_url:
            print(f"  Laptop > {top_name}: {top_url}")
        else:
            print(
                f"WARN no category URL found: Laptop > {top_name}",
                file=sys.stderr,
            )
        ...
        top = {
            "name": top_name,
            "slug": top_slug,
            "labels": ["Laptop", top_name],
            "path": ["laptop", top_slug],
            "url": top_url,
            "depth": 2,
            "order": top_order,
            "products": set(),
        ...
        for child_order, child_name in enumerate(child_names, start=1):
            ...
            if not child_url:
                ...
                child_url = find_exact_link(client, LAPTOP_URL, child_name)
            ...
            if child_url:
                ...
                print(
                    f"WARN no child URL found: Laptop > {top_name} > {child_name}",
                    file=sys.stderr,
                )
            ...
            nodes.append(
                {
                    "name": child_name,
                    "slug": child_slug,
                    "labels": ["Laptop", top_name, child_name],
                    "path": ["laptop", top_slug, child_slug],
                    "url": child_url,
                    "depth": 3,
...
def build_membership(
    ...
    print("\nCrawling Laptop hierarchy for product mapping...")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Download Star Tech Laptop images into hierarchy folders."
    )
    ...
    parser.add_argument(
        "--manifest",
        default="startech_laptop_image_manifest.json",
    ...
    print(f"Reading Laptop master catalog: {LAPTOP_URL}")
    master, reported_total = crawl_listing(client, LAPTOP_URL, verbose=True)

    if not master:
        print("ERROR: No Laptop products found.", file=sys.stderr)
        return 2
    ...
    for idx, (product_url, product_name) in enumerate(selected_items, start=1):
        ...
        if args.category and (
            ...
            category_path = ["laptop", args.category, "_uncategorized"]
