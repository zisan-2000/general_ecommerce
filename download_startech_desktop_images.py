...
def crawl_listing(
    ...
    *,
    ...
    if reported_total is not None and len(products) != reported_total:
        raise RuntimeError(
            f"Incomplete catalog at {base_url}: expected {reported_total} "
            f"unique products, found {len(products)}"
        )
...
def find_named_category_link(
    ...
    # Resolve within the actual menu branch. URL prefixes cannot distinguish
    # these branches: many child category URLs live directly under the domain.
    desktop_menu = None
    for anchor in soup.select("a.nav-link[href]"):
        if norm_text(anchor.get_text(" ", strip=True)) == "desktop":
            desktop_menu = anchor.find_parent("li")
            break
    if desktop_menu is not None:
        scope = desktop_menu
        if parent_label:
            scope = None
            for anchor in desktop_menu.select("a.nav-link[href]"):
                if norm_text(anchor.get_text(" ", strip=True)) == norm_text(parent_label):
                    scope = anchor.find_parent("li")
                    break
        if scope is not None:
            for anchor in scope.select("a.nav-link[href]"):
                if norm_text(anchor.get_text(" ", strip=True)) == wanted:
                    full = canonical_url(urljoin(page_url, str(anchor["href"])))
                    if same_site(full):
                        return full
        return None
    if parent_label:
        # Never silently assign a same-named category from a different parent.
        return None
...
def build_category_membership(
    ...
    for node in nodes:
        ...
        try:
            ...
            raise RuntimeError(f"Category crawl failed for {node['url']}: {exc}") from exc
...
def main() -> int:
    ...
    missing = [" > ".join(node["labels"]) for node in nodes if not node.get("url")]
    if missing:
        raise RuntimeError("Category URLs missing: " + "; ".join(missing))

    master_urls = set(master.keys())
    build_category_membership(client, nodes, master_urls)
    if not args.dry_run:
        for node in nodes:
            output_root.joinpath(*node["path"]).mkdir(parents=True, exist_ok=True)
