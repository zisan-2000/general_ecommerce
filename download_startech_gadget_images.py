#!/usr/bin/env python3
"""
Download ALL Star Tech Gadget product primary images into hierarchy folders.
...
Examples:
    ...
    # Mapping only
    python download_startech_gadget_images.py --project-root . --dry-run

    # Test first 10
    python download_startech_gadget_images.py --project-root . --limit 10

    # Download all current /gadget products
    python download_startech_gadget_images.py --project-root .

    # Download one branch
    python download_startech_gadget_images.py --project-root . --category smart-watch
    python download_startech_gadget_images.py --project-root . --category earbuds
    python download_startech_gadget_images.py --project-root . --category power-bank

    # Re-download existing images
    python download_startech_gadget_images.py --project-root . --overwrite

Notes:
- Product count is NOT hardcoded.
- Pagination is read live from Star Tech.
- Existing images are skipped unless --overwrite is used.
- Products are classified by matching Star Tech category pages.
- A manifest is continuously written to:
      startech_gadget_image_manifest.json
"""
...
def main() -> int:
    ...
    ap.add_argument("--manifest", default="startech_gadget_image_manifest.json")
    ...
    print(f"Reading Gadget master catalog: {GADGET_URL}")
    master, reported_total = crawl(client, GADGET_URL, verbose=True)
    ...
    print("\nDiscovering Gadget category URLs...")
    nodes = []
    ...
    for order,(slug,label) in enumerate(CATEGORY_LABELS.items(), start=1):
        ...
        print(f"  Gadget > {label}: {url or 'NOT FOUND'} ({len(matched)} matched)")
        nodes.append({
            "slug":slug,
            "name":label,
            "url":url,
